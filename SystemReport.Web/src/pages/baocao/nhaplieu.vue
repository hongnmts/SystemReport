...
<!--                <multiselect-->
<!--                    :multiple="false"-->
<!--                    v-model="paramMauBieu.kyBaoCao"-->
<!--                    :options="optionKyBaoCao"-->
<!--                    track-by="id"-->
<!--                    label="ten"-->
<!--                    placeholder="Chọn loại ký báo cáo"-->
...
<!--              <template v-if="paramMauBieu.kyBaoCao != '' && paramMauBieu.kyBaoCao != null && paramMauBieu.kyBaoCao != undefined">-->
<!--                <div class="col-3 "  v-if="paramMauBieu.kyBaoCao && paramMauBieu.kyBaoCao.loaiKyBaoCao == 'nam'">-->
              ...
              <div class="col-12">
                ...
                <div class="table-responsive-sm">
                  <b-table
                      ...
                      primary-key="id"
                      :busy.sync="isBusy"
                      tbody-tr-class="b-table-nhaplieu"
                  >
                    <template v-slot:cell(STT)="data">
                      {{ data.index + ((currentPage-1)*perPage) + 1  }}
                    </template>
                    <template v-slot:cell(kyHieu)="data">
                      <div style="text-align: center;" class="px-2">    {{data.item.kyHieu}}</div>
                    </template>
                    ...
                    <template v-slot:cell(showKyBaoCao)="data">
                      <div style="text-align: center;" class="px-2 ">    {{data.item.showKyBaoCao}}</div>
                    </template>
                    <template v-slot:cell(lastStatus)="data">
                      <div style="display: flex; justify-content: center; align-items: center; flex-direction: column">
                        <div>
                          <span v-if="data.item.lastStatus" class="badge" :class="'bg-' + data.item.lastStatus.bgColor"> {{
                              data.item.lastStatus.ten
                            }}</span>
                        </div>
                        <i v-if="data.item.lastStatus != null  && !isEmpty(data.item.lastStatus.content) "
                           v-on:click="handleShowNoiDungTuChoi(data.item)"
                           class="fas fa-question-circle  me-1"
                           :class="checkErrorStatus(data.item.lastStatus.code)?'text-danger':'text-primary'"
                    ...
                    <template v-slot:cell(status)="data">
                      <b-button
                          v-if=" data.item.lastStatus && (data.item.lastStatus.code == 'NL' || data.item.lastStatus.code == 'CBKTTC')"
                          type="button"
                          size="sm"
                          class="btn btn-light btn-danger"
                          data-toggle="tooltip" data-placement="bottom" title="Xử lý mẫu biểu"
                          v-on:click="handleChuyenTrangThai(data.item.lastStatus, data.item.id, data.item)"
                      >
                              ...
                              <div class="row">
                                <div class="col-lg-12 col-md-12 col-sm-12">
                                  ...
                                  <div class="row" v-if="data.item.bangBieus.length > 0">
                                    <div class="col-12 ">
                                      <div class="card   p-3" v-for="(value, index) in data.item.bangBieus" :key="index">
                                        <div class="card-body">
                                          <h4 class="card-title"><h4 class="card-title">#{{index + 1}}. {{value.ten}}</h4></h4>
                                          <p> </p>
                                          <a v-if="data.item.lastStatus && (data.item.lastStatus.code == 'NL')" href="javascript:void(0);" @click="handleThucHienNhapLieu(value.id)" class="btn btn-primary btn-block me-1" size="sm">   <i class="mdi mdi-circle-edit-outline me-1"></i> Nhập liệu</a>
                                          <a href="javascript:void(0);" @click="handleXemBangBieu(value.id)" class="btn w-lg btn-info btn-block" size="sm"> <i class="mdi mdi-eye me-1"></i>Xem bảng biểu</a>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                              ...
                              <div class="row">
                                <div class="col-12">
                                  ...
                                  <div class="table-responsive-sm table-responsive">
                                    <b-table
                                        class="datatabless custom-table"
                                        ...
                                        tbody-tr-class="b-table-history"
                                    >
                                      <template v-slot:cell(STT)="data">
                                      ...
                                      <template v-slot:cell(status)="data">
                                        <div style="display: flex; justify-content: center; align-items: center; flex-direction: column">
                                          <div>
                                      <span v-if="data.item.status" class="badge" :class="'bg-' + data.item.status.bgColor"> {{
                                  ...
                                  <div class="row">
                                    <b-col>
                                      <div>Hiển thị {{historyOption.numberOfElement}} trên tổng số {{historyOption.totalRows}} dòng</div>
                                    </b-col>
                                    <div class="col">
                                      <div
      ...
      <form @submit.prevent="handleSubmit"
        ...
        <div class="row">
          ...
          <div class="col-12 row">
            <div class="col-6">
              <label class="text-left form-label">Từ ngày</label>
              <br />
              <date-picker v-model="model.tuNgay"
                           format="DD/MM/YYYY"
                           v-on:change="handleChoseModelTuNgay"
              >
                <div slot="input" >
            ...
            <div class="col-6">
              <label class="text-left form-label">Đến ngày</label>
              <br />
              <date-picker v-model="model.denNgay"
                           format="DD/MM/YYYY"
                           v-on:change="handleChoseModelDenNgay"
              >
                <div slot="input" >
    ...
    >
      ...
      <div id="dynamic-table1">
        <table class="">
          ...
          <tr>
            <td>{{model.kyHieu}}</td>
            <td style="text-align: center" colspan="4">   <div v-if="model.nam" style="font-style: italic">Kỳ báo cáo: Năm {{model.nam}}</div>
              <div v-if="model.tuNgayDenNgay" style="font-style: italic">({{model.tuNgayDenNgay}})</div></td>
            <td colspan="2">
              <div>
...
<!--    Danh sách mẫu biểu được phân quyền-->
    ...
    >
      <div class="row">
        ...
        <template v-if="modelInputMauBieu.kyBaoCao != '' && modelInputMauBieu.kyBaoCao != null && modelInputMauBieu.kyBaoCao != undefined">
          ...
          <div v-else-if="modelInputMauBieu.kyBaoCao && modelInputMauBieu.kyBaoCao.loaiKyBaoCao == 'thang'" class="col-6">
            <label class="form-label"> Thời gian</label>
            <br />
            <div style="display: flex;">
              <multiselect
          ...
          <div class="col-6 row" v-else-if="modelInputMauBieu.kyBaoCao && modelInputMauBieu.kyBaoCao.loaiKyBaoCao == 'giaidoan'">
            <div class="col-6">
              <label class="text-left form-label">Từ ngày</label>
              <br />
              <date-picker v-model="modelInputMauBieu.tuNgay"
                           format="DD/MM/YYYY"
                           v-on:change="handleChoseTuNgay"
              >
                <div slot="input" >
                  <input v-model="modelInputMauBieu.tuNgay" v-mask="'##/##/####'" type="text" class="form-control" />
                </div>
              </date-picker>
            </div>
            <div class="col-6">
              <label class="text-left form-label">Đến ngày</label>
              <br />
              <date-picker v-model="modelInputMauBieu.denNgay"
                           format="DD/MM/YYYY"
                           v-on:change="handleChoseDenNgay"
              >
                <div slot="input" >
                  <input v-model="modelInputMauBieu.denNgay" v-mask="'##/##/####'" type="text" class="form-control" />
                </div>
              </date-picker>
            </div>
          </div>
        ...
        <table-mau-bieu :refreshTableMauBieu="refreshTableMauBieu" :showModalInputMauBieu.sync="showModalPhanMauBieu" :modelInput.sync="modelInputMauBieu" />
      </div>
    </b-modal>
...
export default {
  ...
  components: {History, TableMauBieu, Layout, PageHeader, Switches, Multiselect, DatePicker},
  data(){
    return{
      title: "Nhập liệu",
      items: [
  ...
  return (!str || str.length === 0 );
    ...
    handleChoseModelDenNgay(val){
      this.model.denNgay = this.choseDate(val) ;
    },
    handleChoseModelTuNgay(val){
      this.model.tuNgay = this.choseDate(val) ;
    },
    handleChoseDenNgay(val){
      this.modelInputMauBieu.denNgay = this.choseDate(val) ;
    },
    handleChoseTuNgay(val){
      this.modelInputMauBieu.tuNgay = this.choseDate(val) ;
    },
    choseDate(value){
     ...
     if (
       ...
       await this.$store.dispatch("mauBieuStore/changeStatus", this.modelTrangThai).then((res) => {
         if (res.resultCode === 'SUCCESS') {
           ...
           this.modelTrangThai = trangThaiModel.currentBaseJson()
           this.$refs.tblList?.refresh();
         }
   ...
   async handleChooseMauBieu(id){
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("mauBieuStore/getPagingParamsNhapLieu", params)
...
@media only screen and (max-width: 768px) {
  ...
  Label the data
  */
  .b-table-nhaplieu > td:nth-of-type(1):before {
    content: "STT";
    font-weight: bold;
    color: #00568c;
  }

  .b-table-nhaplieu>td:nth-of-type(2):before {
    content: "Ký hiệu";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(3):before {
    content: "Tên biễu mẫu";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(4):before {
    content: "Loại biễu mẫu";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(5):before {
    content: "Kỳ báo cáo";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(6):before {
    content: "Trạng thái";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(7):before {
    content: "";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-nhaplieu>td:nth-of-type(8):before {
    content: "";
    font-weight: bold;
    color: #00568c;
  }
  ...
  /*
  Label the data history
  */
  .b-table-history > td:nth-of-type(1):before {
    content: "STT";
    font-weight: bold;
    color: #00568c;
  }

  .b-table-history>td:nth-of-type(2):before {
    content: "Thời gian";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-history>td:nth-of-type(3):before {
    content: "Người thao tác";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-history>td:nth-of-type(4):before {
    content: "Hành động";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-history>td:nth-of-type(5):before {
    content: "Trạng thái";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-history>td:nth-of-type(6):before {
    content: "Nội dung";
    font-weight: bold;
    color: #00568c;
  }
  .b-table-history>td:nth-of-type(7):before {
    content: "";
    font-weight: bold;
    color: #00568c;
  }
}
.b-table-nhaplieu:hover{
  background: #ddd !important;
  position: relative;
  z-index: 99;
  --bs-table-accent-bg: #ddd;
}
.table-striped > tbody > .b-table-nhaplieu:nth-of-type(odd):hover{
  --bs-table-accent-bg: #ddd;
}
...
@media only screen and (max-width: 768px){
  ...
  #dynamic-table1 thead tr:nth-child(even){background-color: #f2f2f2}
  #dynamic-table2 thead tr:nth-child(even){background-color: #f2f2f2}
