...
import {donViTinhModel} from "@/models/donViTinh";
...
export default {
  ...
  watch: {
    ...
    showModal(status) {
      if (status == false) this.model = donViTinhModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("donViTinhStore/getById", id).then((res) => {
        if (res.resultCode === 'SUCCESS') {
          this.model = donViTinhModel.fromJson(res.data);
          this.showModal = true;
        } else {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("donViTinhStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("donViTinhStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("donViTinhStore/create", donViTinhModel.toJson(this.model)).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.$refs.tblList.refresh()
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("donViTinhStore/getPagingParams", params)
              ...
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-button
                      ...
                      size="sm"
                  >
                    <i class="mdi mdi-plus me-1"></i> Tạo Đơn vị tính
                  </b-button>
                  <b-modal
                      v-model="showModal"
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tên Đơn vị tính</label>
