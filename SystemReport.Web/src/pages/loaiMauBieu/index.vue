...
import {loaiMauBieuModel} from "@/models/loaiMauBieuModel";
...
export default {
  ...
  data() {
    return {
      ...
      submitted: false,
      model: loaiMauBieuModel.baseJson(),
      listCoQuan: [],
      listRole: [],
  ...
  watch: {
    ...
    showModal(status) {
      if (status == false) this.model = loaiMauBieuModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("loaiMauBieuStore/getById", id).then((res) => {
        if (res.resultCode === 'SUCCESS') {
          this.model = loaiMauBieuModel.fromJson(res.data);
          this.showModal = true;
        } else {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("loaiMauBieuStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("loaiMauBieuStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("loaiMauBieuStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.$refs.tblList.refresh()
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("loaiMauBieuStore/getPagingParams", params)
              ...
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-button
                      ...
                      size="sm"
                  >
                    <i class="mdi mdi-plus me-1"></i> Tạo loại mẫu biểu
                  </b-button>
                  <b-modal
                      v-model="showModal"
                      title="Thông tin loại mẫu biểu"
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tên loại mẫu biểu</label>
                            ...
                            <input
                                ...
                                type="text"
                                class="form-control"
                                placeholder="Nhập tên loại mẫu biểu"
                                :class="{
                                'is-invalid':
                            ...
                            >
                              Tên loại mẫu biểu không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-12">
