...
{
    public class QuanLyKHService : BaseService, IQuanLyKHService
    {
        private DataContext _context;
        private BaseMongoDb<QuanLyKH, string> BaseMongoDb;
        private IMongoCollection<QuanLyKH> _collection;
        private IDbSettings _settings;
        private ILoggingService _logger;

        public QuanLyKHService(ILoggingService logger, IDbSettings settings, DataContext context,
            IHttpContextAccessor contextAccessor)
            : base(context, contextAccessor)
        {
            _context = context;
            BaseMongoDb = new BaseMongoDb<QuanLyKH, string>(_context.QuanLyKH);
            _collection = context.QuanLyKH;
            _settings = settings;
        ...
        public async Task<QuanLyKH> Create(QuanLyKH model)
        {
            if (model == default)
            {
            ...
            var entity = new QuanLyKH
            {
                TenDeTai = model.TenDeTai,
                ChuTri = model.ChuTri,
                ChuNhiem = model.ChuNhiem,
                LinhVuc = model.LinhVuc,
                QuyetDinhPDKQ = model.QuyetDinhPDKQ,
                NgayPDKQ = model.NgayPDKQ,
                NguonNSNN = model.NguonNSNN,
                NguonKhac = model.NguonKhac,
                NgayBatDau = model.NgayBatDau,
                NgayKetThuc = model.NgayKetThuc,
                NgayGiaHan = model.NgayGiaHan,
                DangThucHien = model.DangThucHien,
                NgayNghiemThu = model.NgayNghiemThu,
                XepLoai = model.XepLoai,
                QuyetDinhCQ = model.QuyetDinhCQ,
                NgayChuyenGiao = model.NgayChuyenGiao,
                DonViTiepNhan = model.DonViTiepNhan,
        ...
        public async Task<QuanLyKH> Update(QuanLyKH model)
        {
            if (model == default)
            {
                throw new ResponseMessageException()
            ...
            var entity = _context.QuanLyKH.Find(x => x.Id == model.Id).FirstOrDefault();
            if (entity == default)
            {
                throw new ResponseMessageException()
                    .WithCode(EResultResponse.FAIL.ToString())
            ...
            entity.QuyetDinhPDKQ = model.QuyetDinhPDKQ;
            entity.NgayPDKQ = model.NgayPDKQ;
            entity.NguonNSNN = model.NguonNSNN;
            entity.NguonKhac = model.NguonKhac;
            entity.NgayBatDau = model.NgayBatDau;
            entity.NgayKetThuc = model.NgayKetThuc;
            entity.NgayGiaHan = model.NgayGiaHan;
            entity.DangThucHien = model.DangThucHien;
            entity.NgayNghiemThu = model.NgayNghiemThu;
            entity.XepLoai = model.XepLoai;
            entity.QuyetDinhCQ = model.QuyetDinhCQ;
            entity.NgayChuyenGiao = model.NgayChuyenGiao;
            entity.DonViTiepNhan = model.DonViTiepNhan;
        ...
        public async Task Delete(string id)
        {
            ...
            var entity = _context.QuanLyKH.Find(x => x.Id == id && x.IsDeleted != true).FirstOrDefault();
            if (entity == default)
            {
                throw new ResponseMessageException()
                    .WithCode(EResultResponse.FAIL.ToString())
        ...
        public async Task<List<QuanLyKH>> Get()
        {
            return await _context.QuanLyKH.Find(x => x.IsDeleted != true).SortByDescending(x => x.CreatedAt)
                .ToListAsync();
        }

        public async Task<QuanLyKH> GetById(string id)
        {
            return await _context.QuanLyKH.Find(x => x.Id == id && x.IsDeleted != true)
                .FirstOrDefaultAsync();
        }
